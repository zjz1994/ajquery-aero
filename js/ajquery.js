...
function updatebrowser() {
    ...
    if (br.pathcurrent != '%20') {
        ...
        $('#browse_parent').click(function() {
            br_offsets[br.pathcurrent] = $('#browse_dlg').scrollTop();
            retrievebrowserstate(br.parent);
        });
    } else {
        brparams.cmdenqueue = '';
    ...
    for (var i = 0; i < len; ++i) {
        ...
        tmp += ['<td nowrap class="bt-r" style="text-align: right">', row.fs, '</td>',
            ...
            '<td nowrap class="bt-r">', row.ft, '</td></tr>'
        ].join('');
    }
...
function updateplaylist() {
    ...
    var len = fb.playlist.length;
    if (pstfb && (fb.playlistActive == pstfb.playlistActive) && (fb.playlistPage == pstfb.playlistPage) && (fb.playingItem == pstfb.playingItem)) { return; }
    for (var i = 0, k = (fb.playlistPage - 1) * fb.playlistItemsPerPage; i < len; ++i, ++k) {
        ...
        ta.push(['<div id="i', k, '" class="pl_row ', cl_r.join(' '), '">',
            ...
            '<div class="pl_c3 ', cl_3.join(' '), '">', row.l, '</div></div>'
        ].join(''));
    }
...
function updatetabs() {
    if (pstfb && (fb.playlistActive == pstfb.playlistActive) && (JSON.stringify(fb.playlists) == JSON.stringify(pstfb.playlists))) { return; }
...
function retrievestate(cmd, p1) {
    ...
    $.getJSON(['/ajquery/', cmd, p1, 'param3=js/state.json'].join(''), function(data, status) {
        ...
        if (fb.isPlaying == '1' && fb.helper1 == '' || fb.isEnqueueing == '1')
            ...
            $("#brbg").css("background-image") == "url(" + fb.albumArt + ")" ? console.log(1) : $("#brbg").css("background-image", "url(" + fb.albumArt + ")");
...
$(function() {
    $(document).ready(function() {
        ...
        setInterval(function() { retrievestate(); }, 3000);
    });
});